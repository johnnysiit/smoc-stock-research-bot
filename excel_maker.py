...
def dict_get(ticker,runmode):
    ...
    all_data_dict = dm.data_dictionary(ticker,runmode)
    # note = dict()
    # note["OMBDA_"] = "ROE在最早年份不准确"
    # note["Operating Income"] = "ROE in the earliest year is not accurate "
    # note["Reconciled Depreciation"] = "If Interest Expense is 1, then EI and EIC could not be apply"
    # note["Tax Provision"] = "如果Interest Expense为1，则EI和EIC不能使用"
    # all_data_dict["Note"]=note
        #获取该年所需数据
    return all_data_dict

def table_generate(ticker,output_mode,runmode):
    current_path = os.path.dirname(os.path.abspath(__file__))
    data_list = (dict_get(ticker,runmode))
...
def main(ticker,output_mode,runmode):
    ...
    table_generate(ticker,output_mode,runmode)
